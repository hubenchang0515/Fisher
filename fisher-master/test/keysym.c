...
// from here: https://github.com/libsdl-org/SDL/blob/120c76c84bbce4c1bfed4e9eb74e10678bd83120/src/video/x11/SDL_x11keyboard.c#L44
static const struct {
    uint32_t keysym;
    int32_t scancode;
} KeySymToSDLScancode[] = {
...
{
    for (size_t i = 0; i < ARRAY_SIZE(KeySymToSDLScancode); i++)
    {
        if (KeySymToSDLScancode[i].scancode == scancode)
            return KeySymToSDLScancode[i].keysym;
    }
